...
def test_authorization_house_email_pass(pdriver):
    """Сценарий авторизации через почту и пароль на сайте Умный дом"""
    # Кликаем на кнопку пойти с паролем
    pdriver.find_element(By.ID, 'standard_auth_btn').click()
    time.sleep(2)
    # Кликаем на кнопку почта
    pdriver.find_element(By.ID, 't-btn-tab-mail').click()
    time.sleep(2)
    # Вводим почту
    pdriver.find_element(By.ID, 'username').send_keys(valid_email)
    # Вводим пароль
...
# pytest -v --driver Chrome --driver-path C:\\Users\\gelin\\PycharmProjects\\Project33.1\\webdriver\\chromedriver.exe test_authorization_house_email_pass.py