...
def test_authorization_mail(driver):
    """ Проверяем авторизацию через соц сеть mail """
    # Кликаем на кнопку mail
    driver.find_element(By.ID, 'oidc_mail').click()
    time.sleep(2) # Ставим таймер на 2 сек для прогрузки контента страницы
    # Вводим email учетной записи
    driver.find_element(By.ID, 'login').send_keys(valid_email)
    # Вводим пароль учетной записи
    driver.find_element(By.ID, 'password').send_keys(valid_password)
    # Кликаем на кнопку войти и разрешить
...
# pytest -v --driver Chrome --driver-path C:\\Users\\gelin\\PycharmProjects\\Project33.1\\webdriver\\chromedriver.exe test_authorization_mail.py