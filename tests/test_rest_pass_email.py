import time
import pytest
from settings import valid_password2, valid_email
...
def test_rest_pass_email(driver):
    """Сценарий восстановления пароля по email"""
    # Кликаем на кнопку забыл парполь
    driver.find_element(By.ID, 'forgot_password').click()
    # Кликаем на кнопку почта
    ...
    time.sleep(2)
    # Кликаем на кнопку по e-mail
    driver.find_element(By.XPATH, '//*[@id="page-right"]/div/div/div/form/div/label[2]/span/span[1]').click()
    # Кликаем на кнопку продолжить
    driver.find_element(By.XPATH, '//*[@id="page-right"]/div/div/div/form/button[1]').click()
    time.sleep(10) # Таймер на ввод кода с почты
    # Вводим новый пароль
...
# pytest -v --driver Chrome --driver-path C:\\Users\\gelin\\PycharmProjects\\Project33.1\\webdriver\\chromedriver.exe test_rest_pass_email.py